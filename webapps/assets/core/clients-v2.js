var clients = {
    ...
    pickClient: ko.observable(""),
    titlepanel: ko.observable(' Show Filter'),
    ...
    bankingDetails: ko.observable(""),
    Notes: ko.observable(""),
    AltNumber: ko.observable(""),
    MessengerId: ko.observable(""),
    Pasport: ko.observable(""),
};
...
clients.reloadGrid = function(){
    $("#MasterGridClientsv2").data("kendoGrid").dataSource.read({
...
// var userid2 = model.User();
// var gc = new GridColumn('role_master_v2', userid2, 'MasterGridClientsv2');
clients.getDataGridClients = function(){
    ...
    $("#MasterGridClientsv2").html("");
    $("#MasterGridClientsv2").kendoGrid({
            dataSource: {
                    transport: {
                        read: {
                            url: url,
    ...
    //$("#MasterGridClientsv2").data('kendoGrid').bind('error', clients.gridError())
}
...
clients.saveData = function(){
    ...
    var param = {
        ...
        IsClearer: "0",
        Notes: clients.Notes(),
        BankingNotes: clients.bankingDetails(),
        AltNumber: clients.AltNumber(),
        MessengerId: clients.MessengerId(),
        Pasport: clients.Pasport(),
    }
...
clients.showTab8 = function(){
    $("#tab8").show();
}

clients.onhide = function(){
    $("#tab8").hide();
}
...
clients.editAccount = function(userName){
    ...
    ajaxPost(url, param, function(res){
        ...
        clients.Notes(d.Notes);
        clients.bankingDetails(d.BankingNotes);
        clients.AltNumber(d.AltNumber);
        clients.MessengerId(d.MessengerId);
        clients.Pasport(d.Pasport);
        if(d.IsClearer == "0"){
            $('#clearer').bootstrapSwitch('state', false);
        }else{
...
clients.toggleFilter = function(){
  ...
  if(clients.titlepanel() == ' Show Filter'){
    clients.titlepanel(' Hide Filter')
  }else{
    clients.titlepanel(' Show Filter')
  }
}
