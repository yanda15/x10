...
rptClients.tradeDate.subscribe(function(value){
    if(model.View() != "false"){
        rptClients.search();
    }
});

rptClients.clients.subscribe(function(value){
    if(model.View() != "false"){
        rptClients.search();
    }
});

rptClients.reportType.subscribe(function(value){
    if(model.View() != "false"){
        rptClients.search();
    }
});
...
rptClients.search = function () {
  var typeReport = rptClients.reportType();
  switch (typeReport) {
    case "All":
      rptClients.rptGetReportAllClients();
      break;
    default:
      break;
  }
...
rptClients.refresh = function () {
  rptClients.tradeDate(new Date());
  rptClients.getClients();
  rptClients.rptGetReportAllClients();
}

rptClients.exportReport = function () {
  rptClients.search();
  var TextHtml = "";
  for (var i in rptClients.lenRpt()) {
  ...
  hiddenField.setAttribute("value", moment(rptClients.tradeDate()).format("YYYY-MM-DD"));
  form.appendChild(hiddenField);
  hiddenField = document.createElement("input");
  hiddenField.setAttribute("type", "hidden");
  hiddenField.setAttribute("name", "Account");
  hiddenField.setAttribute("value", rptClients.clients());
  form.appendChild(hiddenField);
...
rptClients.rptGetReportAllClients = function () {
  var url = "/reportclient/accountsummary";
  var param = {
    TradeDate: moment(rptClients.tradeDate()).format("YYYY-MM-DD"),
    Clients: rptClients.clients(),
  }
  ajaxPost(url, param, function (res) {
    $('#allClientReport').html("");
    $divAllReport = $('#allClientReport');
    var DataRptClientAccounts = res.Data;
    rptClients.lenRpt(DataRptClientAccounts);
    for (var rpt in DataRptClientAccounts) {
      ...
      // ==== Tbody Report Payments and Receipts ============================================================================
        if (DeskFeeData.length != 0) {
          ...
          for (var dfIndex in DeskFeeData) {
            var itemsCurr = DeskFeeData[dfIndex];
            var totAmount = 0;
            var itemsDf = itemsCurr.items;
            for (var itemsDfIndex in itemsDf) {
              ...
              $tr = $("<tr>" +
                    "<td colspan ='5'style='text-align: right; font-size: 10px; font-weight:bold'>TOTAL "+ itemsCurr.Currency +"</td>" +
      ...
      // ==== Tbody Report Summary ==========================================================================================
        ...
        for (var SumIndex in SummaryAccount) {
          $tdCurrCode = $("<td style='text-align: center; font-weight: bold;  width: 12%; font-size: 10px;text-align: right; padding-right:20px'>" + SummaryAccount[SumIndex].CurrencyCode + "</td>");
          $tdCurrCode.appendTo($trHederCurrCodeValue);
        }

        $trFXSPORTRATE = $("<tr></tr>");
        $trFXSPORTRATE.appendTo($subSummaryAccountvalue);
        $tdFXSPORTRATE = $("<td style='border-right: 1px solid; font-weight: bold;text-align: left; font-size: 11px; text-align: left; padding-right:20px'>FX SPOT RATE</td>");
...
//    rptClients.ClientSummary($divAllReport, rptClients.clients(), rptClients.tradeDate());
  });
}

rptClients.ClientSummary = function (MainTable, Clients, tradeDate) {
  var param = {
  ...
  ajaxPost(url, param, function (res) {
    ...
    if (SummaryClient.length != 0) {
      ...
      $subSummaryClientvalue = $("<table style='width:100%; border:0'></table>");
      ...
      for (var SumIndex in SummaryClient) {
        $tdCurrCode = $("<td style='text-align: center; font-weight: bold;  width: 15%; font-size: 10px;text-align: right; padding-right:20px'>" + SummaryClient[SumIndex].CurrencyCode + "</td>");
        $tdCurrCode.appendTo($trHederCurrCodeValue);
      }

      $trFXSPORTRATE = $("<tr></tr>");
      $trFXSPORTRATE.appendTo($subSummaryClientvalue);
      $tdFXSPORTRATE = $("<td style='border-right: 1px solid; font-weight: bold;text-align: left; width: 15%;  font-size: 11px; padding-right:20px'>FX SPOT RATE</td>");
      $tdFXSPORTRATE.appendTo($trFXSPORTRATE);
...
rptClients.getClients = function () {
  var param = {
  };
  ...
  ajaxPost(url, param, function (res) {
    ...
    rptClients.listClients(_.sortBy(listClients, 'text'));
    rptClients.clients(listClients[1].text);
  });
}
