...
rptClientsSumm.search = function () {
  var typeReport = rptClientsSumm.reportType();
  switch (typeReport) {
    case "All":
      rptClientsSumm.ClientSummary();
      break;
    default:
      break;
  }
...
rptClientsSumm.refresh = function () {
  rptClientsSumm.tradeDate(new Date());
  rptClientsSumm.getClients();
  rptClientsSumm.ClientSummary();
}

rptClientsSumm.tradeDate.subscribe(function(value){
    if(model.View() != "false"){
        rptClientsSumm.search();
    }
});

rptClientsSumm.clients.subscribe(function(value){
    if(model.View() != "false"){
        rptClientsSumm.search();
    }
});

rptClientsSumm.reportType.subscribe(function(value){
    if(model.View() != "false"){
        rptClientsSumm.search();
    }
});

rptClientsSumm.exportReport = function () {
  rptClientsSumm.search();
  var TextHtml = "";
  ...
  hiddenField.setAttribute("value", moment(rptClientsSumm.tradeDate()).format("YYYY-MM-DD"));
  form.appendChild(hiddenField);
  hiddenField = document.createElement("input");
  hiddenField.setAttribute("type", "hidden");
  hiddenField.setAttribute("name", "Account");
  hiddenField.setAttribute("value", rptClientsSumm.clients());
  form.appendChild(hiddenField);
...
rptClientsSumm.ClientSummary = function (){
    var param = {
      TradeDate: moment(rptClientsSumm.tradeDate()).format("YYYY-MM-DD"),
      Clients: rptClientsSumm.clients
    };
    ...
    ajaxPost(url, param, function (res) {
      ...
      $theadMain = $("<thead>" +
              ...
              "<th style='text-align: left; width: 20%'>"+ moment(rptClientsSumm.tradeDate()).format("dddd, DD-MMMM-YYYY") +"</th>" +
              ...
              "<th style='text-align: left'>"+rptClientsSumm.clients()+"</th>" +
              "<th>&nbsp;</th>" +
              "<th>&nbsp;</th>" +
              "</tr>" +
              "<tr>" +
      ...
      if (SummaryClient.length != 0){
        ...
        $trSummaryClient = $("<tr>" +
                "<td colspan='6' style='font-size: 12px;font-weight: bold;color: #3c82be;text-align: center;padding-top: 10px;'>Summary Client</td>" +
                "</tr>");
        ...
        $tr = $("<tr>" +
                ...
                "</tr>" +
                "<tr>" +
                "<td style='text-align: left; font-weight: bold; font-size: 10px' colspan='7'>Clients Summary : " + rptClientsSumm.clients() +"</td>" +
                "</tr>" +
                "<tr>" +
                "<td style='text-align: left; font-size: 10px; font-weight: bold' colspan='7'>Comsolidate Financial Statement as of : " + moment(rptClientsSumm.tradeDate()).format("dddd, DD-MMMM-YYYY") + " </td>" +
                "</tr>");
...
rptClientsSumm.getClients = function () {
  var param = {
  };
  ...
  ajaxPost(url, param, function (res) {
    ...
    rptClientsSumm.listClients(_.sortBy(listClients, 'text'));
    rptClientsSumm.clients(listClients[1].text);
  });
}
